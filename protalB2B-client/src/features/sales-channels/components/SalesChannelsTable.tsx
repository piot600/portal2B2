...
import styles from "./SalesChannelsTable.module.css";
...
export default function SalesChannelsTable({ data }: Props) {
  ...
  if (!data.length)
    return (
      <p className={styles.emptyMessage}>Brak raportów do wyświetlenia.</p>
    );
        ...
        <tbody>
          {data.map((r) => (
            <tr key={r.id}>
              ...
              <td>{r.third_party}</td>
              <td>{r.other}</td>
              <td className={styles.totalCell}>{r.total_sales}</td>
              <td>{r.new_clients}</td>
            </tr>
          ))}
        </tbody>
